...
import {
  IMPORTABLE_PROCEDURE_FIELDS,
  type DedupeStrategy,
  type ImportSummary,
  type ProcedureImportRowInput,
  type SpaceType,
} from "../../types/operations";
import styles from "../prospects/ProspectImportModal.module.css";

const IGNORE = "__ignore__";
const SPACE_TYPES: SpaceType[] = ["maca", "cadeira", "poltrona"];
const NUMBER_FIELDS = new Set<keyof ProcedureImportRowInput>(["points", "spaceMinutes"]);

function guessMapping(header: string): string {
  const normalized = header.trim().toLowerCase();
  const match = IMPORTABLE_PROCEDURE_FIELDS.find(
...
function buildRow(headers: string[], mapping: string[], values: string[]): ProcedureImportRowInput | null {
  ...
  headers.forEach((_, colIndex) => {
    ...
    const key = field as keyof ProcedureImportRowInput;
  ...
  return row as ProcedureImportRowInput;
}

export function ProcedureImportModal({
  onClose,
  onImported,
}: {
  onClose: () => void;
    ...
    () =>
      headers.length
        ? rows
            ...
            .filter((r): r is ProcedureImportRowInput => r !== null)
        : [],
  ...
  async function handleSubmit() {
    ...
    setImporting(true);
    try {
      const result = await operationsApiRepository.bulkImportProcedures(mappedRows, dedupeStrategy);
      setSummary(result);
    } catch (err) {
  ...
  return (
    <div className={styles.overlay} onClick={onClose}>
      <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
        <h2 className={styles.modalTitle}>Importar procedimentos via CSV</h2>
        <p className={styles.modalSubtitle}>
          Suba a planilha, relacione cada coluna com um campo (de/para) e escolha o que fazer
          quando o código já existir cadastrado. Cada linha vira um procedimento com UM trecho de
          espaço — procedimentos com mais de um trecho (ex.: maca + poltrona) precisam ser
          cadastrados pelo formulário.
        </p>
                ...
                {headers.map((header, colIndex) => (
                  <div key={header + colIndex} className={styles.mappingRow}>
                    ...
                    >
                      ...
                      {IMPORTABLE_PROCEDURE_FIELDS.map((f) => (
        ...
        <div className={styles.modalActions}>
          ...
          {summary ? (
            ...
            >
              {importing ? "Importando…" : `Importar ${mappedRows.length || ""} procedimento(s)`}
            </Button>
          )}
        </div>
      </div>
